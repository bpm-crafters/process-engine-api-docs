---
title: Process Engine Adapter CIB Seven Embedded
---
...
Currently, the Process Engine Adapter CIB Seven Embedded supports the following values in task information meta block, mapped from the CIB Seven engine:

The `TaskInformation.getMeta()` provides meta information about the task in form of a `Map<String, String>` for maximum compatibility. The Original Type column
denotes
the real type, you want to access if reading the field. For this purpose, `TaskInformation` offers special access methods `getMetaValueAsOffsetDate` and
`getMetaValueAsStringSet`.
...
| processInstanceId    | String         | Id of the process instance                                                  | a1b2c3d4-...                  |
...
| formKey              | String         | Form key of the user task                                                   | embedded:deployment:form.html |
...
| Key                  | Original Type  | Description                                                | Example                       |
|----------------------|----------------|------------------------------------------------------------|-------------------------------|
| activityId           | String         | Id of the element in BPMN (Task definition key)            | approve_service_task          |
| processDefinitionId  | String         | Id of process definition (given at deployment time)        | approval_process:912834729348 |
| processDefinitionKey | String         | Id of the process element in BPMN (Process Definition key) | approval_process              |
| processInstanceId    | String         | Id of the process instance                                 | a1b2c3d4-...                  |
| tenantId             | String         | Tenant Id                                                  | my_tenant                     |
